...
''');
      ...
      if (from < 32) {
        await _migrateV32SatangColumns();
      }
      if (from < 27) {
...
''');
  ...
  /// Mirrors [_deduplicateBarcodesLower].
  Future<void> _deduplicateSkuLower() async {
    final duplicates = await customSelect('''
  ...
  Future<void> _migrateV32SatangColumns() async {
    ...
    for (final (table, bahtCol, satangCol) in conversions) {
      await _addColumnIfNotExists(table, satangCol, 'INTEGER');
      await _backfillSatangColumn(table, bahtCol, satangCol);
    }
  ...
  Future<void> _backfillSatangColumn(
    String table,
    String bahtCol,
    String satangCol, {
    String? condition,
