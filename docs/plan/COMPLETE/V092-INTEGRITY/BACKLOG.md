...
| V092-D.5 | Fix TestApp: drop `pumpAndSettle` on restart; add stable Keys for the blocking journeys | V092-2 | — | `integration_test/helpers/test_app.dart`: `restartApp` uses `pump` not `pumpAndSettle`; `lib/core/testing/test_keys.dart` owns the stable key catalog | **done** (2026-08-14) |
