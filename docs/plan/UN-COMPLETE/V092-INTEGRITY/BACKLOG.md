...
[ARCH-HARDEN BACKLOG](../ARCH-HARDEN-1.0/BACKLOG.md) · [POST-090 BACKLOG](../POST-090-MANAGE/POST-090-BACKLOG.md)
...
| V092-A.1 | Withdraw “tax invoice” as receipt document type — Tax ID may still print | V092-1 | — | `receipt_document.dart` + PDF + tests + listing do not conflict | todo |
| V092-A.2 | Sync PIN docs: 0.9.1 default-on is not Optional | V092-1 | — | SECURITY / CHANGELOG note / features / STORE_SUBMISSION | todo |
| V092-A.3 | Sync CI/AAB/E2E docs with real YAML (fail-closed, trust blocking, main CI does not run device) | V092-1 | — | SECURITY, STORE_SUBMISSION E4, testing.md, integration_test/README | todo |
| V092-A.4 | Withdraw/fix overclaims: sync-ready, full POS, Void/Refund, schema v28 in SECURITY/features | V092-1 | — | docs PR | todo |
| V092-B.1 | Every path that changes `stock` / `price` / `cost` goes through a use case that calls `requireSensitiveSession()` | V092-1 | — | `update_product` + quick-edit + tests | todo |
| V092-B.2 | Lock on cold start and return-from-background when store PIN is on | V092-1 | — | `main.dart` / `main_shell.dart` + tests | todo |
| V092-B.3 | PIN on CloseDay, report/history export, and settings for discounts / oversell / day-lock / backupEncryption | V092-1 | V092-B.1 partial | domain + settings_sensitive_fields | todo |
| V092-C.1 | Sale / void / adjust / product form: never write stock from a stale cache; `stock = stock ± ?` + `version++` | V092-1 | — | writers + product datasource + overwrite test | todo |
| V092-D.1 | One host integ: EXCLUSIVE 7% + discount + void restock + day-close | V092-2 | V092-C.1 | new file under `test/integration/` + `release-trust.yml` | todo |
| V092-D.2 | Device: History void with a known PIN succeeds once and is recorded | V092-2 | V092-B.2 partial | `RELEASE_1.0_SMOKE` Must #3 or a 0.9.2 addendum | todo |
| V092-D.3 | Sync E2E docs with `ci.yml` / `release-trust.yml` (do not call it soft if trust is hard) | V092-2 | V092-A.3 | testing.md + E2E_* + integration_test/README | todo |
| V092-GATE | G1–G8 in [GATE-TO-TAG.md](./GATE-TO-TAG.md) | V092-4 | Must above | signed checklist | todo |
...
| V092-B.4 | Delete/TTL `pre_restore_*.db` after the new DB opens successfully | V092-1 | — | `backup_restore_service.dart` + tests | todo |
| V092-B.5 | FLAG_SECURE on the PIN settings page (minimum) and keep it on PromptPay | V092-1 | — | `app_lock_settings_page.dart` | todo |
| V092-B.6 | Reject trivial PINs (`123456`, `000000`) | V092-1 | — | `AppLockService` + onboarding | todo |
| V092-C.2 | Dedupe `sku_lower` before unique (v31 if needed), same pattern as barcode | V092-1 | — | `app_database.dart` + migrate test | todo |
| V092-C.3 | Call an idempotent index/trigger set at the end of every `onUpgrade` | V092-1 | V092-C.2 optional | migrate from an old fixture | todo |
| V092-D.4 | Void-after-day-close full stack (use case, not datasource bypass) in trust | V092-2 | V092-D.1 | test + release-trust | todo |
| V092-D.5 | Fix TestApp: drop `pumpAndSettle` on restart; add stable Keys for the 5 core cases | V092-2 | — | `integration_test/helpers/test_app.dart` | todo |
| V092-E.1 | Unlock landscape on tablet (phone may stay portrait) | V092-3 | — | `main.dart` + dual-pane smoke | todo |
| V092-E.2 | Open DB/migrate and build PDF off the UI isolate | V092-3 | — | `database_opener.dart` + receipt service | todo |
| V092-E.3 | Barcode lookup hits the whole catalog, not ProductBloc’s first 500 rows | V092-3 | — | barcode handlers + tests | todo |
...
| V092-B.7 | `resetOnError: false` + separate DB-key vs PIN storage namespaces | Data-loss risk; design first | deferred |
...
| V092-C.4 | Unique barcode/SKU policy after soft-delete | Docs vs index | todo |
...
| V092-D.7 | Add `onboarding_first_sale_test` to the trust list | Small | todo |
...
| **V092-4** | [GATE-TO-TAG.md](./GATE-TO-TAG.md) = **UNLOCKED** |
