...
| **Inventory** | Audit log (SALE, VOID_REVERSAL, ADJUSTMENT_IN/OUT); manual stock adjust sheet with reason — **store PIN** when lock enabled; per-product log viewer |
...
| **Backup export / restore** | WAL checkpoint → DB copy → AES-GCM (PIN ≥ 6, ** **on**) + share; CSV export; reminder. **Same-device in-app restore** of `.enc` / SQLCipher `.db`. Cross-device / after uninstall **not** supported. Schema **v28** |
| **Store PIN lock** | Optional PIN (min 6, PBKDF2); gates void, backup export/restore, stock adjust, CSV import, PromptPay, encryption-off; lockout persists across cold start; session cleared on background |
