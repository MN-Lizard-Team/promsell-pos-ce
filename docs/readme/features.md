...
| **History** | Date-ranged receipt-like sale history with expandable item breakdown, receipt numbers, VOIDED badge, VAT breakdown rows (Subtotal + VAT rate %) when VAT is active, void sale action with reason, notes, and search bar (filter by receipt number, payment method, or amount). **(v0.9.3)** Cursor-based pagination (`querySalesPage`) with composite `(created_at DESC, id)` cursor — items/payments hydrated only for the current page (batched, not N+1) |
| **Report** | Dashboard cards for net revenue (excludes voided), voided summary, payment method breakdown, top 5 products, date filter chip, pull-to-refresh, and empty states. **History sub-tab** (v0.8.9+): `HistoryTabView` as TabBar sub-tab in `ReportPage`. **(v0.9.3)** SQL-level report summary aggregate (`queryReportSummary`) — no `List<Sale>` hydration; payment breakdown chunked at 500 rows. **(v0.9.3)** Bounded streaming CSV export (`exportCsvStream`) — O(pageSize) memory, 10k row hard cap, truncation flag |
...
| **Settings** | Elderly-friendly redesign with larger touch targets (48dp icons, 64dp tiles). Dashboard cards with gradient backgrounds and status badges on every page. Dialog-based visual pickers for language/theme with icon-based option cards. PromptPay ID validation (phone 10 digits / citizen ID 13 digits). Shop Info inline form with live preview and phone auto-format. Backup reminder switch + preset frequency picker (3/7/14/30 days). "Reset to Defaults" confirmation dialog. Root page dashboard with 5 summary badges and grouped sections (`Store & Business`, `Payments`, `System & Data`) with colored status chips on every tile. **v0.7.2**: 3-level hierarchy (Root → SubTopic → Page) with flattened search, backup encryption toggle, theme color tokens (`AppColors`) replacing hardcoded values. **v0.7.1**: Compact Cart Mode toggle, global theme unification (green accent `#00C853`, dark bg `#0D1117`, 16px card / 12px button radius), readability fixes for dark mode badges and icons. **v0.9.4**: POS-native restyle — teal app-bar chrome with integrated search strip (matches `SaleAppBar`), white hero card with thin border + teal readiness progress bar (0/4 checks), compact 64dp action cards with 12dp radius and thin borders (emphasized = 1.5px teal border), plain `titleMedium/w700` section headers, `surfaceContainerLow` scaffold background; dedicated full-screen `SettingsSearchPage` mirroring `SaleProductSearchPage`; `SettingsThemeExtension` tokens (`cardRadius`, `actionCardRadius`, `actionCardMinHeight`, `statusBadgeRadius`, `heroProgressHeight`); Tabler Icons Plus across all Settings pages (see ADR-037) |
...
| **Backup export / restore** | WAL checkpoint → DB copy → SHA-256 checksum → AES-GCM (PIN ≥ 6, default **on**) + share; CSV export; reminder. **Same-device in-app restore** of `.enc` / SQLCipher `.db` with staged file swap + rollback. **(v0.9.3)** `BackupMetadata` sidecar (`.meta.json` with schema/app version, size, checksum, encrypted flag); `BackupRestoreService` candidate validation (schema, integrity, FK) before swap; pre-restore backup for rollback. Cross-device / after uninstall **not** supported. Schema **v32** |
| **Recovery kit** (v0.9.3, **code complete — device validation pending**) | `RecoveryKitService` exports the SQLCipher key as a `.promkey` file wrapped with AES-256-GCM + PBKDF2-HMAC-SHA256 (100K iterations, isolate-backed). Import unwraps and installs the key into platform secure storage. Min secret length 8. File format: `[uint32 headerLen][JSON header][salt(16)][nonce(12)][ciphertext+tag]`. Must be paired with a DB backup to restore data. On-device cross-device restore (D2) is not yet tested |
| **DB lifecycle** (v0.9.3) | `MigrationSafetyService` — free-space preflight (2× DB / 50 MB floor) + `migration_status.json` tracking for interrupted-migration detection. `WalCheckpointService` — PASSIVE at 10 MB (safe during transactions), TRUNCATE at 50 MB (exclusive lock, backup/day-close). `DatabaseHealthService` — `DatabaseHealthReport` with file sizes, schema version, integrity check, 512 MB guardrail (400 MB approaching) |
...
| **Onboarding** (v0.9.4) | First-launch 4-step wizard: shop info (name, address, phone, tax ID + receipt header preview) → preferences (locale, currency, theme, date format) → business/tax (VAT mode radio cards + rate + PromptPay ID) → done (gradient hero + accent-stripe summary card). Gradient-hero + accent-stripe visual language: deep-teal gradient hero, pill-style step indicator, accent-stripe section cards with tinted icon wells, cleaner bottom bar with back chevron + full-width primary CTA (accent orange on last step). Settings sheet with pill section headers. Tabler Icons Plus throughout. Prompts for store PIN creation on finish or skip (see ADR-037) |
...
| **Material 3** | Shared theme tokens and responsive UI primitives. **v0.9.4**: Tabler Icons Plus (`tabler_icons_plus` ^3.44.0) as the app-wide icon library across 100+ files; `SettingsThemeExtension` tokens — Settings-active (`cardRadius`, `actionCardRadius`, `actionCardMinHeight`, `statusBadgeRadius`, `heroProgressHeight`) + onboarding-retained (`heroGradientStart/End`, `accentStripeWidth`, `pillRadius`) |
