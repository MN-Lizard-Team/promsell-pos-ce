...
| Feature | Description |
|---------|-------------|
| **Sale** | Catalog + full-page cart review (`CartReviewPage`); `paymentLocked` freeze on confirm; multi-tender + PromptPay; payable SSOT (`SalePayableCalculator` / Money satang). Park/saved bills, per-item & cart discount, day-closed banner. Checkout failure unlocks cart without clearing lines |
...
| **Backup export / restore** | WAL checkpoint → DB copy → AES-GCM (PIN ≥ 6, default **on**) + share; CSV export; reminder. **Same-device in-app restore** of `.enc` / SQLCipher `.db`. Cross-device / after uninstall **not** supported. Schema **v30** |
| **Store PIN lock** | Required on new install (min 6, PBKDF2); may be turned off later in Settings if the code still allows. Gates void, backup export/restore, AdjustStock, CSV import, PromptPay, encryption-off — **not** product-form stock/price. Lockout persists across cold start; session cleared on background |
...
| **Database** | Drift + **SQLCipher** — **16 tables**, schema **v30** (`sale_payments` multi-tender), UUID PKs; Money satang in domain / REAL baht on disk |
