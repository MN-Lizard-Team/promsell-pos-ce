...
| `DraftCarts` table schema | Run `dart run build_runner build`; bump schema version + add migration in `app_database_migrations.dart` (or a new `app_database_migration_vXX_*.dart` `part of` file for large migrations) + legacy fixture test |
