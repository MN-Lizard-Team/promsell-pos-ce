...
When you change a table, bump `schemaVersion` in `app_database.dart` and add a migration step in `onUpgrade`. Current schema version: **18** (v0.8.6). See the [Drift migration docs](https://drift.simonbinder.eu/Migrations/) for details.
...
Promsell has **1259 automated tests** covering domain logic, state management, data access, services, widgets, integration, stress testing, and localization parity.
