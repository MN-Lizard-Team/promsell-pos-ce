...
Use `DateFormatter` (`lib/core/utils/date_formatter.dart`) for all date/time display. It maps app language codes to intl locales automatically:

```dart
import 'package:promsell_pos_ce/core/utils/date_formatter.dart';

// "27 ก.ค. 2026 • 10:37" (TH) / "27 Jul 2026 • 10:37" (EN)
DateFormatter.formatDateTime(context, dateTime);

// "27 ก.ค. 2026" (TH) / "27 Jul 2026" (EN)
DateFormatter.formatDate(context, dateTime);

// "10:37" (both)
DateFormatter.formatTime(context, dateTime);

// "27 ก.ค. 2026 • 10:37 • บิลที่ 3"
DateFormatter.formatDateTimeWithSuffix(context, dateTime, suffix);
```

To add a new locale, update `_localeFor()` in `date_formatter.dart`:
...
└── tables/
    ...
    ├── restaurant_tables_table.dart
    ├── product_option_groups_table.dart
    ├── product_options_table.dart
    ├── customers_table.dart
    ├── promotions_table.dart
    └── product_audit_table.dart
```
...
When you change a table, bump `schemaVersion` in `app_database.dart` and add a migration step in `onUpgrade`. Current schema version: **30**. See the [Drift migration docs](https://drift.simonbinder.eu/Migrations/) for details.
...
Promsell has a multi-layer automated test suite (domain, blocs, datasources, widgets, integration). Run with `flutter test --exclude-tags stress`.
...
Datasource and integration tests use a real SQLite database in memory via `sqlcipher_flutter_libs` (FFI). This provides true SQL execution without disk I/O:
...
Add `sqlcipher_flutter_libs` to `dev_dependencies` in `pubspec.yaml` for this to work on desktop test runners.
...
```bash
adb shell pm clear com.promsell.promsell_pos_ce
```
