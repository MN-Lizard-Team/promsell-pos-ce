...
```
lib/core/database/
├── app_database.dart       # Database class, schema v28, migration, indexes, seed
...
When you change a table, bump `schemaVersion` in `app_database.dart` and add a migration step in `onUpgrade`. Current schema version: **28**. See the [Drift migration docs](https://drift.simonbinder.eu/Migrations/) for details.
