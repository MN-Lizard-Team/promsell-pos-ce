# RELEASE 0.9.2 — Device Smoke Sheet

**Status:** Pass required before 0.9.2 tag  
...
> **Do not call this a 1.0 Go.** The `RELEASE_1.0_SMOKE.md` sheet is still **No-Go** (M2, prod AAB belong to 1.0). This sheet only covers the 0.9.2 scope: cold-start + PIN + void + stock restore.
...
|---|------|---------------------|--------|-------|
| 1 | Cold start + PIN unlock (V092-B.2 landed) | **Pass** | ☐ | App locks on cold start when PIN is enabled. Unlock with test PIN. |
...
<sub>Promsell POS CE · v0.9.2 · Device Smoke · V092-D.2</sub>
